...
def argparser():
    ...
    parser.add_argument('-data_folder', default='/data/jbwu_data/xiazhi_data/new_data_1000/anno_part10', type=str)
...
def save_mask_nii(pred, height, width, save_path, file_name):
    for _idx in range(4):
        _pred = pred[_idx, :, :]
        _pred = cv2.resize(_pred, (width, height), interpolation=cv2.INTER_NEAREST)
        ...
        _right_pred[:, :_mid] = 0
        _save_mask_nii(_left_pred, str(_idx + 1), os.path.join(save_path, file_name, file_name + '_L' + str(_idx + 1) + '.nii.gz'))
        _save_mask_nii(_right_pred, str(_idx + 7), os.path.join(save_path, file_name, file_name + '_R' + str(_idx + 7) + '.nii.gz'))
...
def main():
    ...
    test_dataset = SegDataset(
        ...
        data_transform=valid_transform, fold_index=fold_id, scale=scale_mode, data_format='dcm'
    )
